...
def render_read_mgr_inst(prot_id: str, prot_ids: dict, db: dict) -> dict:
    ...
    for rp in prot_ids[prot_id]["ar"]:
        ...
        # template cleanup, ignore for functionality
        db[rp]["read_template"] = "    " + db[rp]["read_template"].replace(
            "\n", "\n    "
        )
        db[rp]["read_template"] = db[rp]["read_template"][:-5]
        if rp == "axi":
            db[rp]["read_template_multiple_ports"] = "    " + db[rp][
                "read_template_multiple_ports"
            ].replace("\n", "\n    ")
            db[rp]["read_template_multiple_ports"] = db[rp][
                "read_template_multiple_ports"
            ][:-5]
...
"""
        ...
        if rp == "axi":
            for i in range(2):
                read_port_context["port"] = str(i)
                res[rp] += (
                    Template(db[rp]["read_template_multiple_ports"]).render(
                        **read_port_context
                    )
                    + "\n\n"
                )
...
# In: prot_ids {r, w, rw, ar (all_read_prots), aw (all_write_prots), used (used_protocols)}
# In: db {axi, obi, axil, axis, init}, all Dicts from YAML files
# In: tpl_file: Filename of Template File
# Out: Rendered SV code
def render_transport_layer(prot_ids: dict, db: dict, tpl_file: str) -> str:
    ...
    for prot_id in prot_ids:
        ...
        with open("debug.output.txt", "a") as debug_file:
            debug_file.write(", ".join(prot_ids[prot_id]["ar"]) + "\n")
            debug_file.write(", ".join(["2" for x in prot_ids[prot_id]["ar"]]) + "\n")
            import json

            debug_file.write(json.dumps(context))
